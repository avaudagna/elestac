...
int main (int argc, char **argv) {
	...
	t_list* cpus_conectadas = list_create();
	t_list* consolas_conectadas = list_create();
	...
	while(1){
		...
		select (maxSocket+1, &allSockets, &checkClientCtrlC, NULL, NULL);
		for(i=0;i<consolesOnline;i++){
			if(FD_ISSET(consoleSockets[i], &checkClientCtrlC)){
				...
				if((read_size = recv(consoleSockets[i], global_buffer_4, 2, 0)) > 0){
					if((strcmp(global_buffer_4, "C1")) == 0 ){
						...
						while( (read_size = recv(consoleSockets[i], global_buffer_4 , 2 , 0)) > 0 ) {
		...
		*/
		...
		if (FD_ISSET(consoleServer, &allSockets)){
			...
			if (aceptado<1){
				...
				if(recv(aceptado, &global_buffer_4, 1, 0)>0){
					if (strcmp(global_buffer_4, "0") == 0){
						list_add(consolas_conectadas,&aceptado);
						/*
						...
						*/
						...
						if(code_pages>0){
							...
							printf(" El PID %d con status %d y PC %d esta listo.\n", newPCB.pid,newPCB.status,newPCB.program_counter);
